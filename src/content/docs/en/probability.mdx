...
As promised, here we are, the **event space**, the punchline is that the **event space** is just a $\sigma-algebra$-algebra, therefore the definition of $\sigma$-algebra is the definition of event space. 🤯
...
With these two concepts -- [sample space](#sample-space) and [event space](#event-space) -- we can construct what is called a "measurable space", which is for by a pair $(\Omega, \ssA)$, where:
- $\Omega$ is the sample space, representing the set of all possible outcomes.
- $\ssA$ is the event space, a collection of subsets of $\Omega$ that is closed under countable unions, countable intersections, and complements—formally, a $\sigma$-algebra.


In measure theory, this pair $(\Omega, \ssA)$ defines a structure -- measurable space -- on which a measure can be meaningfully defined. This framework allows for the rigorous assignment of values (such as probabilities) to subsets of $\Omega$ that belong to the $\sigma$-algebra $\ssA$.

| Measure Theory   | Probability Theory    | Description                                                    |
|------------------|-----------------------|----------------------------------------------------------------|
| abstract space   | sample space $\Omega$ | all possible states $w \in \Omega$                             |
| $\sigma$-algebra | event space $\ssA$    | set of all possible events $\sE \in \ssA$ that one can measure |
...
<MathDefinition
    ...
    definition={`
Is a function that maps from a $\\sigma$-algebra to the positive reals: $\\mu: \\ssA \\rightarrow \\sR^{+}$ such that:
(i) $\\mu(\\emptyset) = 0$
(ii) $\\\mu(\\bigcup_{i}^{\\infty}\\sE_i ) = \\sum_{i}^{\\infty} \\mu(\\sE_i)$ for a **pairwise disjoint** $\\sE_i \\in \\ssA, i \\in \\sN$ ($\\sigma$-additivity)
`
}
/>

**measure space** is a triplet $(\sS, \ssA, \mu)$, where $(\sS, \ssA)$ is a **measurable space** and $\mu$ is a measure on $\ssA$.

<MathDefinition
    concept="Probability measure"
    definition={`
A probability measure defined on a $\\sigma$-algebra $\\ssA$ of $\\Omega$ **is a measure**, denoted with $P \\,:\\, \\rightarrow [0,1]$ that satisfies:
(i) $\\P(\\Omega) = 1$
(ii) $\\\mu(\\bigcup_{i}^{\\infty}\\sE_i ) = \\sum_{i}^{\\infty} \\mu(\\sE_i)$ for a **pairwise disjoint** $\\sE_i \\in \\ssA, i \\in \\sN$ ($\\sigma$-additivity)
`
}
...
Probability space is a measure space $(\sS, \ssA, P)$, where $P$ is a probability measure on $\ssA$.

For intuitio about functio see the note [On Functions](functions)
