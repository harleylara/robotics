...
For DroneCAN support:
- STM32L431: Cortex-M4F at 80 MHz, up to 256 KB flash / 64 KB SRAM
- STM32G431: Cortex-M4F at 170 MHz / 213 DMIPS, 128 KB flash / 32 KB SRAM split across SRAM and CCM SRAM, FDCAN, rich analog, 4 comparators, op-amps, dual ADCs, CORDIC/FM‌AC math accelerators

Scoping the design intentions:
1. single motor ESC: although I feel tended to do a 4-in-1, this is my first time designing a ESC, so i want to first build some confidence and get the core principels in place, after that it can be scalled up to 4-in-1.
2. firmware: I want to get fimiliar with AM32 as ESC firmware.
3. protocols exploration: DSHOT and DroneCAN for ESC.
3. future vision: I want to explore making the ESC FOC capable, beacuse I would like to try to make a gimbal controller.

The best-fit for my intentions: [`STM32G431CB`](https://www.st.com/en/microcontrollers-microprocessors/stm32g431cb.html) the 48-UFQFPN package.

Electrical requeriments:
- Operational voltage range: 2S-6S
...
2. Conduction loss: Rds(on) at high temperature
...
My selection is the: `IAUCN04S7N004`
